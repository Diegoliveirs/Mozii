...
test.beforeAll(async () => {
  ...
  // escrita em movies agora só via RPC validada (010_hardening.sql)
  await api('/rest/v1/rpc/upsert_movie', a.token, {
    method: 'POST',
    body: JSON.stringify({ p_tmdb_id: 550, p_title: 'Fight Club' }),
  })
...
test('movies: leitura compartilhada, escrita direta bloqueada (010_hardening)', async () => {
  // leitura segue aberta por design: cache público de dados do TMDB
  const visto = await rows(await api('/rest/v1/movies?tmdb_id=eq.550', cToken))
  expect(visto, 'movies segue legível por qualquer autenticado').toHaveLength(1)

  // mas INSERT/UPDATE diretos foram revogados — só a RPC upsert_movie escreve
  const patch = await api('/rest/v1/movies?tmdb_id=eq.550', cToken, {
    method: 'PATCH',
    headers: json,
    body: JSON.stringify({ title: 'Vandalizado' }),
  })
  expect(patch.status, 'UPDATE direto em movies deve ser rejeitado').toBeGreaterThanOrEqual(400)

  const insert = await api('/rest/v1/movies', cToken, {
    method: 'POST',
    body: JSON.stringify({ tmdb_id: 999999, title: 'Filme falso' }),
  })
  expect(insert.status, 'INSERT direto em movies deve ser rejeitado').toBeGreaterThanOrEqual(400)

  // e o registro original permanece intacto
  const [movie] = await rows(await api('/rest/v1/movies?tmdb_id=eq.550', bToken))
  expect((movie as { title: string }).title).toBe('Fight Club')
})
